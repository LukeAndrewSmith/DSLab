...
class EdgeProcessor():
    ...
    def __getEdges(self, candidateEdges, cfg):
        candidateEdgesMask = self.__getCandidateEdgesMask(candidateEdges)
        ...
        return self.identifyEdges(candidateEdgesMask, shapes, cfg)
    

    def __getCandidateEdgesMask(self, edges):
        # converts to binary for further processing:
        # assumes edges to be min=0, max=? prob 255
        candidateEdgesMask = np.around(
            edges/np.max(edges), decimals=0).astype(int)
        return candidateEdgesMask


    def identifyEdges(self, candidateEdgesMask, shapes, cfg):
        ...
        for shape in filteredShapes:
            edge = Edge(shape, candidateEdgesMask.shape)
